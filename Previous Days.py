...
def preprocess(days):
    ...
    while iteration < numOfDataEntries:
        previous.append(reversed_df[iteration:days + iteration, 1])
        forecast.append(reversed_df[iteration + days:+ iteration + days + forecast_days, 1])
        iteration += 1
...
def print_accuracy(accuracies, steps_array):
    for value, step_num in zip(accuracies, steps_array):
        print(str(int(step_num)) + " Previos Days: " + str(round(value*100, 2)) + "%")
...
for step in steps:
    ...
    activation = 'relu'
    model = Sequential()
    model.add(Dense(256, input_dim=int(step)))
