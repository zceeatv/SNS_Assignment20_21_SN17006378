...
for step in steps:
    ...
    ax.plot_date(dates[start_date:start_date + len(predictions)], predictions[:, -1]/1e6, xdate=True, label='Prediction',linestyle='-', marker=' ', linewidth=2)
    ax.plot_date(dates[start_date:start_date + len(predictions)], actual[:, -1]/1e6, xdate=True, label='Actual', linestyle='-', marker=' ', linewidth=2)
